...
do
	echo -ne "\033[0KGenerating animated cursor pixmaps... $i / 8 \\r"
	...
	if [ "$DIR1X/default-$i.png" -ot $RAWSVG ] ; then
		inkscape -i default-$i -d 96  -f $RAWSVG -e "$DIR1X/default-$i.png" > /dev/null
	fi

    if [ "$DIR1_5X/default-$i.png" -ot $RAWSVG ] ; then
        inkscape -i default-$i -d 144 -f $RAWSVG -e "$DIR1_5X/default-$i.png" > /dev/null
	fi

	if [ "$DIR2X/default-$i.png" -ot $RAWSVG ] ; then
		inkscape -i default-$i -d 192 -f $RAWSVG -e "$DIR2X/default-$i.png" > /dev/null
	fi
...
do
    echo -ne "\033[0KGenerating animated cursor pixmaps... $i / 16 \\r"
